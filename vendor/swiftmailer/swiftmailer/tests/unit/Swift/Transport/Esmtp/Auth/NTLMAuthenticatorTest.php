...
class Swift_Transport_Esmtp_Auth_NTLMAuthenticatorTest extends Swift_Tests_SwiftUnitTestCase
{
	...
	public function testMessage1Generator ()
	{
		...
		$this->assertEqual ($this->_message1, bin2hex ($message1), '%s: We send the smallest ntlm message which should never fail.'
		);
	}
	...
	public function testLMv1Generator ()
	{
		...
		$this->assertEqual ($lmv1, bin2hex ($lmv1Result), '%s: The keys should be the same cause we use the same values to generate them.'
		);
	}
	...
	public function testLMv2Generator ()
	{
		...
		$this->assertEqual ($lmv2, bin2hex ($lmv2Result), '%s: The keys should be the same cause we use the same values to generate them.'
		);
	}
	...
	public function testNTLMv1Generator ()
	{
		...
		$this->assertEqual ($ntlm, bin2hex ($ntlmResult), '%s: The keys should be the same cause we use the same values to generate them.'
		);
	}
	...
	public function testNTLMv2Generator ()
	{
		...
		$this->assertEqual ($ntlm2, bin2hex ($ntlmResult), '%s: The keys should be the same cause we use the same values to generate them.'
		);
	}
	...
	public function testMessage3v1Generator ()
	{
		...
		$this->assertEqual ($message3T, bin2hex ($message3), '%s: We send the same information as the example is created with so this should be the same'
		);
	}
	...
	public function testMessage3v2Generator ()
	{
		...
		$this->assertEqual ($this->_message3, bin2hex ($message3), '%s: We send the same information as the example is created with so this should be the same'
		);
	}
	...
	public function testGetDomainAndUsername ()
	{
		...
		$this->assertEqual ('DOMAIN', $domain, '%s: the fetched domain did not match'
		);
		$this->assertEqual ('user', $user, '%s: the fetched user did not match'
		);
	}
	...
	public function testGetDomainAndUsernameWithExtension ()
	{
		...
		$this->assertEqual ('domain.com', $domain, '%s: the fetched domain did not match'
		);
		$this->assertEqual ('user', $user, '%s: the fetched user did not match'
		);
	}
	...
	public function testSuccessfulAuthentication ()
	{
		...
		$this->assertTrue ($ntlm->authenticate ($this->_agent, $username . '@' . $domain, $secret, $this->hex2bin ("30fa7e3c677bc301"), $this->hex2bin ("f5ce3d2401c8f6e9")), '%s: The buffer accepted all commands authentication should succeed'
		);
	}
	...
	public function testAuthenticationFailureSendRsetAndReturnFalse ()
	{
		...
		$this->assertFalse ($ntlm->authenticate ($this->_agent, $username . '@' . $domain, $secret, $this->hex2bin ("30fa7e3c677bc301"), $this->hex2bin ("f5ce3d2401c8f6e9")), '%s: Authentication fails, so RSET should be sent'
		);
	}
