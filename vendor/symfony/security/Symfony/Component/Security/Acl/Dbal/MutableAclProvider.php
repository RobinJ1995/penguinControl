...
class MutableAclProvider extends AclProvider implements MutableAclProviderInterface, PropertyChangedListener
{
	...
	{
		return sprintf (
			'DELETE FROM %s WHERE object_identity_id = %d', $this->options['entry_table_name'], $oidPK
		);
	}
	...
	{
		return sprintf (
			'DELETE FROM %s WHERE id = %d', $this->options['entry_table_name'], $acePK
		);
	}
	...
	{
		return sprintf (
			'DELETE FROM %s WHERE id = %d', $this->options['oid_table_name'], $pk
		);
	}
	...
	{
		return sprintf (
			'DELETE FROM %s WHERE object_identity_id = %d', $this->options['oid_ancestors_table_name'], $pk
		);
	}
...
QUERY;
		...
		return sprintf (
			$query, $this->options['entry_table_name'], $classId, null === $objectIdentityId ? 'NULL' : intval ($objectIdentityId), null === $field ? 'NULL' : $this->connection->quote ($field), $aceOrder, $securityIdentityId, $mask, $this->connection->getDatabasePlatform ()->convertBooleans ($granting), $this->connection->quote ($strategy), $this->connection->getDatabasePlatform ()->convertBooleans ($auditSuccess), $this->connection->getDatabasePlatform ()->convertBooleans ($auditFailure)
		);
	}
	...
	{
		return sprintf (
			'INSERT INTO %s (class_type) VALUES (%s)', $this->options['class_table_name'], $this->connection->quote ($classType)
		);
	}
	...
	{
		return sprintf (
			'INSERT INTO %s (object_identity_id, ancestor_id) VALUES (%d, %d)', $this->options['oid_ancestors_table_name'], $objectIdentityId, $ancestorId
		);
	}
...
QUERY;
		...
		return sprintf (
			$query, $this->options['oid_table_name'], $classId, $this->connection->quote ($identifier), $this->connection->getDatabasePlatform ()->convertBooleans ($entriesInheriting)
		);
	}
	...
	protected function getInsertSecurityIdentitySql (SecurityIdentityInterface $sid)
	{
		...
		return sprintf (
			'INSERT INTO %s (identifier, username) VALUES (%s, %s)', $this->options['sid_table_name'], $this->connection->quote ($identifier), $this->connection->getDatabasePlatform ()->convertBooleans ($username)
		);
	}
	...
	{
		return sprintf (
			'SELECT id FROM %s WHERE class_id = %d AND %s AND %s AND ace_order = %d', $this->options['entry_table_name'], $classId, null === $oid ?
				$this->connection->getDatabasePlatform ()->getIsNullExpression ('object_identity_id') : 'object_identity_id = ' . intval ($oid), null === $field ?
				$this->connection->getDatabasePlatform ()->getIsNullExpression ('field_name') : 'field_name = ' . $this->connection->quote ($field), $order
		);
	}
	...
	protected function getUpdateObjectIdentitySql ($pk, array $changes)
	{
		...
		return sprintf (
			'UPDATE %s SET %s WHERE id = %d', $this->options['oid_table_name'], implode (', ', $changes), $pk
		);
	}
	...
	protected function getUpdateAccessControlEntrySql ($pk, array $sets)
	{
		...
		return sprintf (
			'UPDATE %s SET %s WHERE id = %d', $this->options['entry_table_name'], implode (', ', $sets), $pk
		);
	}
